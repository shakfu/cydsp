"""Tests for cydsp.envelopes module (signalsmith envelopes, LFOs, smoothing)."""
...
class TestCubicLfo:
    def test_construction(self):
        lfo = cydsp.envelopes.CubicLfo()
        assert lfo is not None

    def test_construction_with_seed(self):
        lfo = cydsp.envelopes.CubicLfo(42)
        assert lfo is not None

    def test_set_and_next(self):
        lfo = cydsp.envelopes.CubicLfo(0)
    ...
    def test_deterministic_with_seed(self):
        lfo1 = cydsp.envelopes.CubicLfo(123)
        lfo1.set(0.0, 1.0, 0.01)
        out1 = lfo1.process(128)

        lfo2 = cydsp.envelopes.CubicLfo(123)
...
class TestBoxFilter:
    def test_construction(self):
        bf = cydsp.envelopes.BoxFilter(64)
    ...
    def test_resize(self):
        bf = cydsp.envelopes.BoxFilter(16)
        bf.resize(128)
        bf.set(64)
...
class TestBoxStackFilter:
    def test_construction(self):
        bs = cydsp.envelopes.BoxStackFilter(64)
        assert bs is not None

    def test_construction_with_layers(self):
        bs = cydsp.envelopes.BoxStackFilter(64, 3)
        assert bs is not None
    ...
    def test_smoother_than_single_box(self):
        ...
        bf = cydsp.envelopes.BoxFilter(64)
        bf.set(16)
        bs = cydsp.envelopes.BoxStackFilter(64, 4)
        bs.set(16)
...
class TestPeakHold:
    def test_construction(self):
        ph = cydsp.envelopes.PeakHold(64)
...
class TestPeakDecayLinear:
    def test_construction(self):
        pd = cydsp.envelopes.PeakDecayLinear(64)
    ...
    def test_decay_behavior(self):
        pd = cydsp.envelopes.PeakDecayLinear(128)
        pd.set(64)
